...
### The contents of this file (excluding comments) were last edited on 2021/10/06
## AUTHOR INFORMATION:
### GitHub:           @MathMakesArt
...
## ADDITIONAL AUTHOR NOTES (Not a part of above information)
## The contents of this specific file are based on the SmartPy FA2 template, distributed by SmartPy on their website (https://smartpy.io)
## Modifications have been made to facilitate new operations related to arbitrary on-chain data storage, but the contract should remain FA2-compliant.
...
class Error_message:
    ...
    def onchain_not_enabled(self):   return self.make("ONCHAIN_NOT_ENABLED")
    def onchain_key_undefined(self): return self.make("ONCHAIN_KEY_UNDEFINED")
...
class FA2_core(sp.Contract):
    def __init__(self, config, metadata, **extra_storage):
        ...
        self.init(
            ...
            metadata = metadata,
            onchain_data = self.config.my_map(tkey=sp.TString, tvalue=sp.TBytes),
...
class FA2_onchain_data(FA2_core):
    
    ## Functions facilitating updates to lazily-stored entrypoints
    @sp.entry_point(lazify = False)
    def update_ep_set_onchain_data(self, ep):
        sp.verify(self.is_administrator(sp.sender), message = self.error_message.not_admin())
        sp.set_entry_point("set_onchain_data", ep)
    @sp.entry_point(lazify = False)
    def update_ep_set_onchain_data_type(self, ep):
        sp.verify(self.is_administrator(sp.sender), message = self.error_message.not_admin())
        sp.set_entry_point("set_onchain_data_type", ep)
    @sp.entry_point(lazify = False)
    def update_ep_del_onchain_data_type(self, ep):
        sp.verify(self.is_administrator(sp.sender), message = self.error_message.not_admin())
        sp.set_entry_point("del_onchain_data", ep)
    
    @sp.entry_point(lazify = True)
    def set_onchain_data(self, k, v, typeString=""):
        sp.verify(self.is_administrator(sp.sender), message = self.error_message.not_admin())
        sp.verify(self.config.enable_onchain_data, message = self.error_message.onchain_not_enabled())
        self.data.onchain_data[k] = v
        self.data.onchain_data_types[k] = typeString
        # TODO: Add a reference for typeString (e.g. clarify that empty means interpret as bytes)
        
    @sp.entry_point(lazify = True)
    def set_onchain_data_type(self, k, typeString):
        sp.verify(self.is_administrator(sp.sender), message = self.error_message.not_admin())
        sp.verify(self.config.enable_onchain_data, message = self.error_message.onchain_not_enabled())
        self.data.onchain_data_types[k] = typeString
        # TODO: Add a reference for typeString (e.g. clarify that empty means interpret as bytes)
    ...
    @sp.entry_point(lazify = True)
    def del_onchain_data(self, k):
        sp.verify(self.is_administrator(sp.sender), message = self.error_message.not_admin())
        sp.verify(self.config.enable_onchain_data, message = self.error_message.onchain_not_enabled())
        sp.verify(self.data.onchain_data.contains(k), message = self.error_message.onchain_key_undefined())
        del self.data.onchain_data[k]
        del self.data.onchain_data_types[k]


class FA2(FA2_change_metadata, FA2_token_metadata, FA2_mint, FA2_administrator, FA2_pause, FA2_onchain_data, FA2_core):
    ...
    @sp.offchain_view(pure = True)        
    def view_onchain_data(self, k):
        sp.verify(self.config.enable_onchain_data, message = self.error_message.onchain_not_enabled())
        sp.verify(self.data.onchain_data.contains(k), message = self.error_message.onchain_key_undefined())
        sp.result(self.data.onchain_data[k])
    
    @sp.offchain_view(pure = True)        
    def view_onchain_data_type(self, k):
        sp.verify(self.config.enable_onchain_data, message = self.error_message.onchain_not_enabled())
        sp.verify(self.data.onchain_data_types.contains(k), message = self.error_message.onchain_key_undefined())
        sp.result(self.data.onchain_data_types[k])
...
def environment_config():
    return FA2_config(
        ...
        enable_onchain_data = global_parameter("enable_onchain_data", True),
    )
...
if "templates" not in __name__:
    ...
    if not global_parameter("only_environment_test", False):
        ...
        ##add_test(FA2_config(non_fungible = True, add_mutez_transfer = True), is_default = not sp.in_browser)
